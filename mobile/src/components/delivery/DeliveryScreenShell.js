...
import DeliveryMenuSheet from "@/src/components/delivery/DeliveryMenuSheet";
import { useAppBack } from "@/src/utils/navigation";
import { theme } from "@/src/theme";

export default function DeliveryScreenShell({
  children,
  onBack,
  showBack = true,
  showMenu = false,
  ...
  const defaultBack = useAppBack("/delivery-dashboard");
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {footer}
      <DeliveryMenuSheet visible={menuVisible} onClose={() => setMenuVisible(false)} />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  headerSectionTall: { minHeight: 300 },
